...
$filePath = $_SERVER['DOCUMENT_ROOT'] . '/xml/' . 'export_new.csv'; // Путь до файла, куда записываем данные из инфоблока, от корня сайта
...
$IBLOCK_ID = 7;
$SECTION_ID = 81; // Устанавливаем ID инфоблока, с которым хотим работать
...
$arSelect = Array("ID", "NAME", "DETAIL_PICTURE", "PRICE", "PROPERTY_NOVINKI", "PROPERTY_CML2_ARTICLE", "CATALOG_QUANTITY", "PROPERTY_RRTS", "PRICE", "CURRENCY", "PROPERTY_CML2_MANUFACTURER", "PROPERTY_STARAYA_TSENA_KH2", "PROPERTY_MATERIAL", "PROPERTY_RAZMER", "PROPERTY_BREND", "PROPERTY_KOLICHESTVO_V_UPAKOVKE","PROPERTY_CML2_BAR_CODE", "PROPERTY_MINIMALNAYA_PARTIYA","SECTION_ID","IBLOCK_SECTION_ID"); // Массив возвращаемых полей элемента, где слово PROPERTY_ указывает на свойство элемента инфоблока
$arFilter = Array("IBLOCK_ID"=> $IBLOCK_ID, "ACTIVE_DATE"=>"Y", "SECTION_ID"=>array(88,87),  "ACTIVE"=>"Y"); // Выбираем только активные элементы, которые хранятся в инфоблоке с ID 7
// Где Section_id код раздела(ов) каталога
...

{
   ...
   {
	   $exportDATA = array($arFields['ID'], $arFields['NAME'], $domain . CFile::GetPath($arFields["DETAIL_PICTURE"]) , $arFields[PROPERTY_NOVINKI_VALUE], $arFields[PROPERTY_CML2_ARTICLE_VALUE], $arFields['CATALOG_QUANTITY'], $arFields[PROPERTY_RRTS_VALUE],$arFields[PROPERTY_STARAYA_TSENA_KH2_VALUE], $arFields[PROPERTY_RRTS_VALUE]/2 , $arFields[PROPERTY_CML2_MANUFACTURER_VALUE], $arFields[PROPERTY_MATERIAL_VALUE],$arFields[PROPERTY_RAZMER_VALUE],$arFields[PROPERTY_BREND_VALUE],$arFields[PROPERTY_KOLICHESTVO_V_UPAKOVKE_VALUE], $arFields[PROPERTY_CML2_BAR_CODE_VALUE], $arFields[PROPERTY_MINIMALNAYA_PARTIYA_VALUE], $arFields["SECTION_ID"]); // Присваиваем набор полей и свойств инфоблока переменной $exportDATA
